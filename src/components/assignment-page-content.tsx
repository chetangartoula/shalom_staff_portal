...
export function AssignmentPageContent({ report, allGuides, allPorters, initialAssignments }: AssignmentPageContentProps) {
    ...
    const filteredGuides = useMemo(() => {
        return allGuides.filter(guide =>
            guide.name.toLowerCase().includes(guideSearch.toLowerCase()) && guide.status === 'Available'
        );
    }, [allGuides, guideSearch]);

    const filteredPorters = useMemo(() => {
        return allPorters.filter(porter =>
            porter.name.toLowerCase().includes(porterSearch.toLowerCase()) && porter.status === 'Available'
        );
    }, [allPorters, porterSearch]);
    ...
    const TeamMemberSelection = ({ title, items, selectedIds, onSelect, search, onSearch, icon: Icon }: any) => (
        <Card>
            <CardHeader>
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto pr-2 border rounded-md p-2">
                    {items.map((item: any) => (
                        <div key={item.id} className="flex items-center space-x-2 p-2 rounded-md hover:bg-muted">
                            ...
                            />
                            <Label htmlFor={`${title}-${item.id}`} className="flex-grow cursor-pointer">
                                {item.name}
                            </Label>
                        </div>
                    ))}
        ...
        <div className="space-y-8">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Assign guides and porters for trek <span className="font-bold text-primary">{report.trekName}</span> (Group: {report.groupName}).
                    </CardDescription>
                </CardHeader>
            </Card>
