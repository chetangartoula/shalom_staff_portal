...
export function Sidebar({ className, isCollapsed, onAddTrekClick }: SidebarProps) {
    ...
    const NavLink = ({ item }: { item: typeof navItems[0] }) => {
        ...
        const linkContent = (
             <span className={cn(
                ...
                isActive && "text-sidebar-foreground bg-primary/10",
    ...
    return (
        <div className={cn("flex flex-col", className)}>
            <div className={cn("hidden md:flex h-14 items-center border-b px-4 lg:h-[60px] lg:px-6", isCollapsed && "h-[60px] justify-center px-2")}>
            ...
            <div className={cn("mt-auto p-4 space-y-2 border-t w-full", isCollapsed && "p-2 space-y-0")}>
                 {isCollapsed ? (
                    <TooltipProvider>
                        <Tooltip>
